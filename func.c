...
void struct_input(){								// input.txt파일 읽어 구조체에 데이터 넣기
	...
	FILE *read,*write;
	char ch;
	int count,i,start,number_count;
	char temp[20];
	memset(temp,0,20);
	write=fopen("output.txt", "w");
	read=fopen("input.txt", "r");
	count=i=start=number_count=0;
	if (read == NULL){
	...
	while (1){
		...
		if(ch==' '){								// 공백
			if(start==2 && count<=2)	count++;
			temp[i]=ch;
			i++;
			continue;
		}
		else if((ch=='-'&&start==2)&&(count<=2)){	// 휴대폰번호 입력중이면서 '-'라면, 
			count++;
			temp[i]=ch;
			i++;
			continue;
		}
		else if((ch==' '&&start==2)&&(count<=2)){
			count++;
			temp[i]=ch;
			i++;
			continue;
		...
		if(ch==10&&start==2){
			blank_clear(temp);
			struct_data_save(p,1,temp);
			if(p!=person_head){
				pp->next=p;
				pp=pp->next;
			}
			memset(temp,0,20);
			i=count=start=number_count=0;
		}
	}
	fclose(read);
	fclose(write);
...
void argc_check(int argc,char* argv[]){
	switch(argc){
		...
		default:
			...
			else if(strcmp(argv[1],"--help")==0)	argc0_help(argv);
